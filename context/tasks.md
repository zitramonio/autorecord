...
- Текущие задачи:
  - Корневой git checkpoint `a97cd2c initial stable version` создан; рабочее дерево корневого репозитория было чистым после commit.
  - `.gitignore` настроен и сохраняет локальные артефакты вне git: `.worktrees/`, publish/build output, `Autorecord.lnk`, секреты и временные файлы.
...
- Выполнено:
  - Корневой репозиторий уже был инициализирован в `C:\Projects\autorecord`.
  - `.gitignore` расширен для .NET/WPF проекта, Python workers, локальных артефактов, секретов и Windows/editor noise.
  - Создан commit `a97cd2c initial stable version`.
- Текущий статус:
  - Корневой репозиторий на ветке `master`.
  - Рабочее дерево после commit было чистым.
  - Документация Hugging Face token и изображения в `docs/` сохранены в git.
- Следующий шаг:
  - Продолжить ручную GUI-проверку релизного сценария Autorecord и затем делать checkpoint изменений в `.worktrees/mvp`, если проверка успешна.

## Sync 2026-05-14

- Выполнено в `.worktrees/mvp`: добавлено сохранение технических дорожек записи:
...
- Текущие задачи:
  ...
  - Проверить, что автотранскрибация после записи по-прежнему использует текущую выбранную ASR-модель и combined MP3.
  - Позже оценить, даст ли использование `system`/`mic` дорожек улучшение диаризации.
- Выполнено:
  - После закрытия приложения `scripts\publish.ps1` в `.worktrees/mvp` успешно пересобрал актуальный publish.
...
- Следующий шаг:
  - Запустить приложение через корневой ярлык, сделать короткую запись и проверить наличие технических дорожек.

## Sync 2026-05-14

- Выполнено:
  - Создан `README.md` с описанием проекта для GitHub.
  - README фиксирует продуктовую цель, возможности, локальный privacy-подход, стек, документацию, статус и ограничения.
- Текущий статус:
  - В корневом git есть новый untracked `README.md`.
  - Изменение документальное; код и архитектура не менялись.
- Следующий шаг:
  - Проверить README перед публикацией.
  - Затем решить, нужен ли отдельный раздел с инструкциями по сборке/запуску для публичного GitHub.

## Sync 2026-05-14

- Выполнено:
  - README исправлен: календарный источник описан как любой календарь с iCal/.ics-ссылкой.
  - `context/project.md` и `context/decisions.md` синхронизированы с этим scope.
- Текущий статус:
  - Код менять не потребовалось: `CalendarSyncService` уже работает с произвольной iCal/.ics-ссылкой.
  - Изменения остаются документальными.
- Следующий шаг:
  - Финально просмотреть README перед публикацией на GitHub.
