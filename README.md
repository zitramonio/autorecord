...
## Публичная сборка без моделей

Публичный установщик Autorecord не содержит ASR- и diarization-модели. После установки приложение предлагает скачать их отдельно:

- GigaAM v3 — обязательная модель для транскрибации.
- Pyannote Community-1 — опциональная модель для разделения на спикеров.

Если GigaAM v3 не скачана, транскрибация не запускается. Если скачана только GigaAM v3, приложение расшифровывает запись единым текстом без разделения на спикеров.
...
- Публичный установщик не включает модели GigaAM v3 и Pyannote Community-1; их нужно скачать из интерфейса приложения.
